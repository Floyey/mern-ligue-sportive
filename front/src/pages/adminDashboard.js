...
export default function Dashboard() {
          ...
          columns: [
            {
              ...
              data: null,
              orderable: false,
              render: (data) => `<input class="user-checkbox" value="${data._id}" type="checkbox" />`
            },
          ...
          columns: [
            {
              ...
              data: null,
              orderable: false,
              render: (data) => `<input class="product-checkbox" value="${data._id}" type="checkbox" />`
            },
            { title: "Nom", data: "name" },
  ...
  useEffect(() => {
    $(usersTableRef.current).on('click', '#user-check-all', function () {
      ...
      $(usersTableRef.current).find('.user-checkbox').prop('checked', isChecked);
    });

    $(productsTableRef.current).on('click', '#product-check-all', function () {
      const isChecked = $(this).prop('checked');
      $(productsTableRef.current).find('.product-checkbox').prop('checked', isChecked);
    });
  }, [view, users, products]);
